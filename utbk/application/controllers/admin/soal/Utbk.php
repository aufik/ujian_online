...
class Utbk extends CI_Controller {
    ...
    {   parent::__construct();
		...
		$this->load->model('admin/soal/utbk_model', 'dbObject');
		/* session kcfinder */
/* session kcfinder */
	...
	public function index()
	{
		...
		$this->load->view('admin/soal/utbk/list', $data);
		...
		if($this->dbObject->delete_general('tb_soal', $where)===TRUE)		// using direct parameter
			{
				...
				<script> 
					...
					location.replace("<?=base_url()?>admin/soal/utbk"); 
				</script>
				<?php
		...
		if($param=='do_update'){
			...
			if($this->dbObject->update_general("tb_soal", 'soal_id', $param2, $data)===TRUE)		// using direct parameter
			{
				...
				<script> 
					...
					location.replace("<?=base_url()?>admin/soal/utbk/update/<?=$param."/".$param2?>"); 
				</script>
				<?php
			}
		}else{
			...
			$this->load->view('admin/templates/header');
            $this->load->view('admin/templates/sidebar');
            $this->load->view('admin/soal/utbk/update', $data);
            $this->load->view('admin/templates/footer');
		}
	}
    public function tambah($param=""){
        ...
        if($param=='do_create'){
			...
			$this->load->view('admin/templates/header');
            $this->load->view('admin/templates/sidebar');
            $this->load->view('admin/soal/utbk/tambah', $data);
			$this->load->view('admin/templates/footer');
			

			// $this->load->view('admin/soal/utbk/editors');
		}
	}
	function tambah_jurusan(){
		...
		if($this->dbObject->create_general("tb_soal_mapel", $data)===TRUE)		// using direct parameter
		{
			...
			<script> 
				...
				location.replace("<?=base_url()?>admin/soal/utbk/tambah/"); 
			</script>
			<?php
			//redirect('master/jabatan','refresh');
		}
