...
class Siswa extends CI_Controller {
    ...
    {   parent::__construct();
		...
		$this->load->model('sekolah/akademik/siswa_model', 'dbObject');
    }
	public function index()
	{
		$data['siswa'] = $this->dbObject->join_general_sekolah('tb_sekolah', 'tb_siswa', 'sekolah_id', 'sekolah_id', $this->session->userdata('sekolah_id'))->result(); 
		$this->load->view('sekolah/templates/header');
		$this->load->view('sekolah/templates/sidebar');
		$this->load->view('sekolah/akademik/siswa/list', $data);
		...
		if($this->dbObject->update_general('tb_siswa', 'siswa_id', $param, $data)===TRUE)		// using direct parameter
			{
				...
				<script> 
					...
					location.replace("<?=base_url()?>sekolah/akademik/siswa"); 
				</script>
				<?php
		...
		if($param=='do_update'){
			...
			if($this->dbObject->update_general("tb_siswa", 'siswa_id', $param2, $data)===TRUE)		// using direct parameter
			{
				...
				<script> 
					...
					location.replace("<?=base_url()?>sekolah/akademik/siswa/update/<?=$param."/".$param2?>"); 
				</script>
				<?php
			}
		}else{
			...
			$this->load->view('sekolah/templates/header');
            $this->load->view('sekolah/templates/sidebar');
            $this->load->view('sekolah/akademik/siswa/update', $data);
            $this->load->view('sekolah/templates/footer');
		}
	}
    public function tambah($param=""){
        if($param=='do_create'){
			...
			$this->load->view('sekolah/templates/header');
            $this->load->view('sekolah/templates/sidebar');
            $this->load->view('sekolah/akademik/siswa/tambah', $data);
            $this->load->view('sekolah/templates/footer');
		}
    }
